...
begin
  ...
  if p_confirmation is distinct from target.title then
    raise exception 'Type the exact entity title to confirm deletion' using errcode = '22023';
  end if;
  ...
  -- Keep endpoints present while assertion lifecycle triggers run, then remove
  -- the edges before deleting the node and its typed extension.
...
revoke all on function public.delete_entity_permanently(uuid, text)
  from public, anon;
grant execute on function public.delete_entity_permanently(uuid, text)
  to authenticated;

comment on function public.delete_entity_permanently(uuid, text) is
  'Permanently deletes one caller-owned entity and its connected graph records after exact-title confirmation.';
